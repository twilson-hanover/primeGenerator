...
public class PrimePrinter {
  public static void main(String[] args) {
    final int numPrimesToGenerate = 1000;
    final int ORDMAX = 30;
    int primes[] = new int[numPrimesToGenerate + 1];
    ...
    while (lastPrimeIndex < numPrimesToGenerate) {
      boolean maybePrime;
      do {
        candidatePrime += 2;
    ...
    while (pageOffset <= numPrimesToGenerate) {
      System.out.print("The First ");
      System.out.print(Integer.toString(numPrimesToGenerate));
      ...
      for (int rowOffset = pageOffset; rowOffset <= pageOffset + rowsPerPage - 1; rowOffset++) {
        for (int col = 0; col <= colsPerPage - 1; col++)
          if (rowOffset + col * rowsPerPage <= numPrimesToGenerate)
